  ...
  async setup(props, { slots, expose }) {
    ...
    const html = computed(() => {
      ...
      if (import.meta.client && !canLoadClientComponent.value) {
        for (const [key2, value] of Object.entries(payloadComponents || {})) {
              ...
              if (availableSlots.value.includes(slot)) {
                teleports.push(
                  createVNode(
                    ...
                    { default: () => (payloadSlots[slot].props?.length ? payloadSlots[slot].props : [{}]).map((data) => slots[slot]?.(data)) }
                  )
                );
              }
            }
            if (import.meta.server) {
              for (const [id2, info] of Object.entries(payloadComponents ?? {})) {
